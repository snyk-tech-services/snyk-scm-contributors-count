...
export const getRepoCommits = async (
  ...
  const data = await fetch(url + "/" + project + "/_apis/git/repositories/" + repo + "/commits?$top=1000&searchCriteria.fromDate=" + threeMonthsDate + "&api-version=4.1", {
    method: 'GET',
...
export const getReposPerProjects = async (
  ...
  const data = await fetch(url + "/" + project + "/_apis/git/repositories?$top=1000000&api-version=4.1", {
    method: 'GET',
    headers: { Authorization: "Basic " + base64.encode(":" + token)}
})
return data;
