...
export abstract class SCMHandlerClass {
    ...
    abstract fetchSCMContributors(snykMonitoredRepos?:string[]): Promise<ContributorMap>

    scmContributorCount = async (url: string, sourceType: SourceType, skipSnykMonitoredRepos: boolean, exclusionFilePath: string, json: boolean ) => {
        ...
        if(!skipSnykMonitoredRepos){    
          snykImportedRepos = await this.retrieveMonitoredRepos(
              url,
              sourceType
            );
            spinner.succeed();
            ...
            const deduppedSnykImportedRepos =
              this.dedupRepos(snykImportedRepos);
