...
export const excludeFromListByEmail = (
  ...
  for (let i = 0; i < emailExclusionList.length; i++) {
    const key = returnKeyIfEmailFoundInMap(
      contributorsMapToReturn,
      emailExclusionList[i],
    );
    if (key) {
...
export const loadListFromFile = (inputFilePath: string): string[] => {
  ...
  try {
    ...
    list = fs
      .readFileSync(exclusionListFullFilepath)
      .toString()
      .split('\n')
      .filter((x) => x);
  } catch (err) {
